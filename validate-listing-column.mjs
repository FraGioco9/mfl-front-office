...
assert.match(core, /const compactTableHeader = window\.matchMedia\("\(max-width: 1366px\)"\)\.matches;/);
assert.match(core, /label\.textContent = column === agentColumn && state\.currentPage === "mfl"/);
assert.match(core, /: mobileTable && column === "listing_price"/);
assert.match(core, /: compactTableHeader\s*\? compactLabel\s*: fullLabel;/);
...
assert.match(bootstrap, /if \(column === agentColumn && normalizedPage === "mfl"\) return "";/);
assert.match(bootstrap, /if \(FIRST_PAINT_HORIZONTAL_MEDIA\.matches && column === "listing_price"\) return "";/);
assert.match(bootstrap, /return FIRST_PAINT_COMPACT_HEADER_MEDIA\.matches \? compactLabel : fullLabel;/);
