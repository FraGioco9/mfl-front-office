...
const payload = { columns: ["player_id", "name", "positions", "age", "overall", "active_contract_nb_matches"], rows: [[1, "First Player", "ST", 23, 80, 12], [2, "Second Player", "GK", 25, 75, 8]], totalRows: 2 };
...
assert.equal(contractInput.value, "12", "Contract must start from the current match clause");
...
assert.equal(body.children[0].children[0].textContent, "First Player");
assert.equal(body.children[0].children[4].children[0].value, "18", "Edited contract must survive local roster re-renders");
assert.equal(payload.rows[0][5], 12, "Editing a planned contract must not mutate canonical data");
...
await complete(requests[2], { ...payload, rows: [[3, "Newest Player", "CM", 20, 70, 6]], totalRows: 1 });
