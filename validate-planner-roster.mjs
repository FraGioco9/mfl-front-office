...
assert.equal(elements.get("plannerTeamCard").className, "myClubCard plannerTeamCard", "Planner must reuse the canonical My Clubs card structure");
...
assert.equal(body.children[0].children[0].className, "plannerPlayerSearchFlagCell");
assert.equal(body.children[0].children[1].textContent, "First Player");
assert.equal(body.children[0].children[2].textContent, "GK", "Planner roster must sort by canonical primary-position order");
assert.equal(body.children[1].children[2].textContent, "ST", "Planner roster must keep forwards after defensive/midfield positions");
assert.equal(body.children[0].children[4].textContent, "80", "Overall dot must not change the displayed roster rating");
assert.equal(body.children[0].children[4].children[0].children[0].className, "tableOverallRarityCircle plannerOverallRarityCircle", "Squad Overall must reuse canonical rarity-dot class");
assert.equal(body.children[0].children[4].children[0].children[0].style.backgroundColor, "#0077ff", "Squad Overall dot must use the canonical rarity color");
assert.equal(body.children[0].children[3].children[0].children[0].textContent, "23", "Age must remain visible in the planned squad");
assert.equal(body.children[0].children[3].children[0].children[1].className.includes("retirementMarker--retiring-2"), true, "Retirement marker must match canonical 1–3 year semantics");
assert.equal(body.children[1].children[3].children[0].children[1].className.includes("newMintMarker"), true, "One-season player must show the New mint marker");
...
const contractControl = body.children[0].children[5].children[0];
...
const secondContractControl = body.children[1].children[5].children[0];
...
assert.equal(body.children[0].children[1].textContent, "First Player");
assert.equal(body.children[0].children[5].children[0].children[0].textContent, "18.25%", "Confirmed contract must survive local roster re-renders");
...
assert.equal(body.children[0].children[1].textContent, "Newest Player", "Old responses must not overwrite the current club");
...
assert.equal(addedRow.children[5].children[0].children[0].textContent, "3.75%", "Added player Contract must also use database value divided by 100");
assert.equal(addedRow.children[3].children[0].children[1].className.includes("newMintMarker"), true, "Added New mint player must keep the marker");
...
const budgetControl = body.children[5].children[5].children[0];
