...
assert.equal(body.children[0].children.length, 8, "Squad rows must include the Slot and nationality columns");
assert.equal(body.children[0].children[0].className, "plannerRosterSlotCell", "Slot is the first squad cell.");
assert.equal(body.children[0].children[0].children[0].className, "plannerRosterSlotBadge", "The Slot badge has a dedicated element.");
assert.equal(body.children[0].children[0].children[0].hidden, true, "Unassigned players do not display a green badge.");
assert.equal(body.children[0].children[0].children[1].textContent, "—", "Unassigned Slot displays a dash.");
assert.equal(body.children[0].children[1].className, "plannerPlayerSearchFlagCell");
assert.equal(body.children[0].children[2].textContent, "First Player");
assert.equal(body.children[0].children[3].textContent, "GK", "Planner roster must sort by canonical primary-position order");
assert.equal(body.children[1].children[3].textContent, "ST", "Planner roster must keep forwards after defensive/midfield positions");
assert.equal(body.children[0].children[5].textContent, "80", "Overall dot must not change the displayed roster rating");
assert.equal(body.children[0].children[5].children[0].children[0].className, "tableOverallRarityCircle plannerOverallRarityCircle", "Squad Overall must reuse canonical rarity-dot class");
assert.equal(body.children[0].children[5].children[0].children[0].style.backgroundColor, "#0077ff", "Squad Overall dot must use the canonical rarity color");
...
assert.equal(["plannerAverageAge","plannerAverageOverall","plannerTotalContracts","plannerBest16Overall","plannerBest11OverallSum"].every(id=>elements.has(id)),true,"A separate five-metric squad summary must exist.");
const metric=id=>elements.get(id).textContent;
assert.equal(metric("plannerTotalContracts"),"20.50%","The initial summary totals both planned contracts.");
assert.equal(metric("plannerAverageOverall"),"77.50","The summary averages all valid squad overalls.");
assert.equal(metric("plannerBest16Overall"),"77.50","Best 16 averages available players when fewer than sixteen are present.");
assert.equal(metric("plannerBest11OverallSum"),"155.00","Best 11 sums available players when fewer than eleven are present.");
assert.equal(metric("plannerAverageAge"),"24.00","The summary averages valid ages.");
...
const secondContractControl = body.children[1].children[6].children[0];
...
assert.equal(metric("plannerTotalContracts"),"26.25%","Confirming a contract must update the standalone summary.");
...
assert.equal(metric("plannerTotalContracts"),"18.25%","Removing a player must update the summary contract total.");
assert.equal(metric("plannerAverageOverall"),"80.00","Removing a player must recompute Overall.");
assert.equal(metric("plannerBest11OverallSum"),"80.00","Removing a player must recompute the top-eleven sum.");
assert.equal(body.children[0].children[2].textContent, "First Player");
...
assert.equal(body.children[0].children[2].textContent, "Newest Player", "Old responses must not overwrite the current club");
...
assert.equal(metric("plannerAverageOverall"),"—","Clearing a club resets old summary data.");
...
assert.equal(addedRow.children[6].children[0].children[0].textContent, "3.75%", "Added player Contract must also use database value divided by 100");
assert.equal(addedRow.children[4].children[0].children[1].className.includes("newMintMarker"), true, "Added New mint player must keep the marker");
...
assert.equal(body.children.reduce((sum, row) => sum + Number.parseFloat(row.children[6].children[0].children[0].textContent), 0).toFixed(2), "100.00", "Planner contract sum must stay within 100%.");
assert.equal(metric("plannerTotalContracts"),"100.00%","Summary contracts must respect the 100% limit.");
assert.equal(metric("plannerBest16Overall"),"65.13","Best 16 average must sort by Overall, not squad Slot order.");
...
assert.equal(searchBody.children[0].children[4].children[0].children[0].className, "tableOverallRarityCircle plannerOverallRarityCircle", "Popup search must display the rarity dot");
assert.equal(searchBody.children[0].children[4].children[0].children[0].style.backgroundColor, "#bebebe", "Popup dot must reflect the searched player's Overall");
