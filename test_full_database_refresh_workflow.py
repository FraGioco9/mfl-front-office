...
class FullDatabaseRefreshWorkflowTests(unittest.TestCase):
    ...
    def test_restore_falls_back_until_a_downloaded_sqlite_database_is_valid(self) -> None:
        self.assertIn("gh run download", self.restore_step)
        self.assertIn("trying the next candidate", self.restore_step)
        self.assertIn("sqlite3.connect", self.restore_step)
        self.assertIn("PRAGMA table_info(players)", self.restore_step)
        self.assertIn("required_columns.issubset(columns)", self.restore_step)
        self.assertIn("SELECT player_id FROM players LIMIT 1", self.restore_step)
