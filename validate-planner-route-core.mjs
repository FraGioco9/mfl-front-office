...
invariant(html.includes('const y = 76 - lineIndex * (66 / (lines.length - 1)) - (midfield ? 3 : 0);')
    && generatedHtml.includes('const y = 76 - lineIndex * (66 / (lines.length - 1)) - (midfield ? 3 : 0);')
...
invariant(html.includes('<select id="plannerFormationSelect" class="plannerFormationSelect" data-mfl-dropdown-enhanced="true">')
    ...
    && styles.includes('.plannerFormationTokenPlus{position:absolute;inset:0;display:block;flex:none;width:45%;height:45%;margin:auto;')
    ...
    && html.includes('const makeFormationSpot = (position, key, goalkeeper = false, starter = null, backups = [], slotLabel = position) => {')
    && generatedHtml.includes('const makeFormationSpot = (position, key, goalkeeper = false, starter = null, backups = [], slotLabel = position) => {')
