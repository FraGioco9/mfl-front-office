...
includes(
  ...
  "Club roster rendering must flow through the canonical Table filter facade rather than the captured generic incremental filter.",
);
includes(
  eagerCore,
...
invariant(
  ...
  "A returned Club roster must bypass generic table restore, commit Club ownership, and render through the canonical Table filter facade in that order.",
);

excludes(
  clubCore,
  "applyFilters = function applyFiltersWithClubRows",
  "Club must not replace the shared Table applyFilters facade with a route-local override.",
);
excludes(
  clubCore,
  "const originalApplyFilters = applyFilters;",
  "Club must not capture a competing Table filter owner.",
);
excludes(
  clubCore,
  "function restoreStandardControls()",
  "Club must not retain cleanup that only existed for the removed filter override.",
);
includes(
  clubCore,
  'if (typeof applyFilters === "function") applyFilters({ save: false, localOnly: true });',
  "Club final rendering must use the canonical shared Table filter facade directly.",
);
...
invariant(
  ...
  "The tracked generated shared runtime must exactly match the normalized eager application core.",
);
...
console.log("Club conflict regression checks passed: one Squad text owner, one public Club navigation workflow for click and refresh, one canonical Table filter owner, non-blocking title preflight, and roster-owned final identity.");
