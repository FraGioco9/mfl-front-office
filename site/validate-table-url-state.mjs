...
for (const [source, label] of [[sharedCore, "canonical Shared source"], [generatedShared, "generated Shared runtime"]]) {
  ...
  invariant(source.includes("const restoredPageState = savedPageState")
    ...
    && source.includes("route.tableFilters = {"), `${label} must resolve URL state before constructing the first incremental request.`);
...
const requestStateIndex = sharedCore.indexOf("route.tableFilters = {", resolveIndex);
