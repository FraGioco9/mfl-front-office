...
function participationQueryParts(clubId) {
  const numericClubId = Number(clubId);
  const parts = [];
  const params = [];
  if (tableExists("competition_standings")) {
    parts.push("SELECT competition_id FROM competition_standings WHERE club_id = ?");
    params.push(numericClubId);
  }
  if (tableExists("competition_matches")) {
    parts.push("SELECT competition_id FROM competition_matches WHERE home_club_id = ? OR away_club_id = ?");
    params.push(numericClubId, numericClubId);
  }
  return { sql: parts.join(" UNION "), params };
}

function participationCompetitionRows(clubId) {
  if (!tableExists("competitions")) return [];
  const participation = participationQueryParts(clubId);
  if (!participation.sql) return [];

  const latest = queryRows(
    `SELECT MAX(season_id) AS seasonId FROM competitions WHERE competition_id IN (${participation.sql})`,
    participation.params,
  );
  const seasonId = Number(latest[0]?.seasonId);
  if (!Number.isInteger(seasonId) || seasonId < FIRST_SEASON_ID) return [];
  ...
  const standingJoin = hasStandings
    ? "LEFT JOIN competition_standings s ON s.competition_id = c.competition_id AND s.club_id = ?"
    : "";
  const params = [];
  if (hasStandings) params.push(Number(clubId));
  params.push(seasonId, ...participation.params);

  return queryRows(
    `SELECT c.competition_id AS competitionId,
     ...
     ${standingJoin}
     WHERE c.season_id = ?
       AND c.competition_id IN (${participation.sql})
     ORDER BY CASE upper(c.type) WHEN 'LEAGUE' THEN 0 ELSE 1 END,
              c.competition_id, stageOrder DESC, groupOrder DESC`,
    params,
  );
}

function explicitCompetitionRows(clubId, rawIds) {
  const ids = parseCompetitionIds(rawIds);
  if (!ids.length || !tableExists("competitions")) return [];
  const placeholders = ids.map(() => "?").join(", ");
  const latest = queryRows(
    `SELECT MAX(season_id) AS seasonId
     FROM competitions
     WHERE competition_id IN (${placeholders}) OR root_competition_id IN (${placeholders})`,
    [...ids, ...ids],
  );
  const seasonId = Number(latest[0]?.seasonId);
  if (!Number.isInteger(seasonId) || seasonId < FIRST_SEASON_ID) return [];
  ...
  const standingJoin = hasStandings
    ? "LEFT JOIN competition_standings s ON s.competition_id = c.competition_id AND s.club_id = ?"
    : "";
  const rootLeagueMembership = hasStandings
    ? "(upper(c.type) <> 'LEAGUE' OR s.club_id IS NOT NULL)"
    : "upper(c.type) <> 'LEAGUE'";
  const params = [];
  if (hasStandings) params.push(Number(clubId));
  params.push(seasonId, ...ids, ...ids, ...ids);
  ...
  return queryRows(
    `SELECT c.competition_id AS competitionId,
     ...
     WHERE c.season_id = ?
       AND (
         c.competition_id IN (${placeholders})
         OR (c.root_competition_id IN (${placeholders}) AND ${rootLeagueMembership})
       )
     ORDER BY CASE WHEN c.competition_id IN (${placeholders}) THEN 0 ELSE 1 END,
...
function currentCompetitions(clubId, rawIds) {
  const participation = participationCompetitionRows(clubId);
  const rows = participation.length ? participation : explicitCompetitionRows(clubId, rawIds);
  const seen = new Set();
  const result = [];
  rows.forEach((row) => {
...
function ownedClubRows(wallet, requestedClubIds = null) {
  ...
  return queryRows(
    `SELECT club_id AS clubId, name,
     ...
     FROM runtime_clubs
     WHERE lower(owner_wallet_address) = ?${idFilter}
...
function myClubsCompetitionsData(signedWallet, clubIds) {
  const wallet = normalizeWalletAddress(signedWallet).toLowerCase();
  const requested = requestedClubIds(clubIds);
  const clubs = ownedClubRows(wallet, requested);
  const competitionsByClub = {};
  clubs.forEach((club) => {
    competitionsByClub[String(club.clubId)] = currentCompetitions(club.clubId, club.currentCompetitionIds);
  });
  return {
    competitionsByClub,
    generatedAt: getGeneratedAt(),
