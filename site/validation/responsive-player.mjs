...
export function validateResponsivePlayer(context) {
  const { sharedTableUi, responsive, controlInteractions, playerInteractions, appCore } = context;
  ...
  includes(playerInteractions, "function compactPlayerPageName(value) {", "Player interactions must own the compact mobile Player name formatter.");
  includes(playerInteractions, 'return fullName.replace(/^(\\S)[^\\s]*\\s+(?:.*\\s)?(\\S+)$/, "$1. $2");', "Mobile Player names must use N. Surname formatting.");
  includes(playerInteractions, "function syncPlayerPageDetails() {", "Player responsive detail presentation must have one interaction-runtime owner.");
  includes(playerInteractions, 'detail.querySelectorAll(".playerTitleName")', "Pending and hydrated Player titles must share the responsive name synchronizer.");
  includes(playerInteractions, "target.dataset.playerFullName = fullName;", "Responsive Player names must retain the full desktop value across breakpoint changes.");
  includes(playerInteractions, 'target.setAttribute("aria-label", fullName);', "Compact Player titles must retain the full accessible name.");

  includes(playerInteractions, 'if (mobile) playerId.removeAttribute("data-tooltip");', "Mobile Player ID copy must not expose the instructional tooltip.");
  includes(playerInteractions, 'else playerId.dataset.tooltip = "Click to copy";', "Desktop Player ID copy tooltip behavior must remain unchanged.");
  includes(appCore, "copyPlayerId(id);", "Suppressing the mobile ID tooltip must not remove ID copying.");

  includes(playerInteractions, 'listing.classList.add("playerListingBadge");', "Player listing badges must expose a mobile-specific presentation hook without changing the shared table badge.");
  includes(playerInteractions, "listing.dataset.tooltip = price;", "Mobile listed Players must expose their exact listing price through the tooltip runtime.");
  includes(playerInteractions, 'listing.setAttribute("role", "button");', "Mobile listing icons must remain keyboard-addressable tooltip controls.");
  includes(playerInteractions, 'listing.setAttribute("aria-label", `Listing price ${price}`);', "Mobile listing icons must expose the exact price accessibly.");
  includes(playerInteractions, 'listing.setAttribute("aria-label", `For Sale at ${price}`);', "Desktop listing accessibility text must be restored when leaving mobile layout.");
  includes(playerInteractions, "syncPlayerPageDetails();", "Player presentation must synchronize both initially and when its DOM changes.");
  includes(playerInteractions, 'playerAttributeViewMutationObserver.observe(detail, { childList: true, subtree: true, characterData: true });', "Player presentation must synchronize pending and hydrated DOM before paint.");
  includes(playerInteractions, 'PLAYER_VIEW_SCROLL_MEDIA.addEventListener("change", onPlayerViewScrollMediaChange);', "Player presentation must resynchronize when crossing the mobile breakpoint.");

  excludes(controlInteractions, "function compactPlayerPageName(value) {", "Universal control interactions must not retain Player-only responsive presentation.");
  excludes(controlInteractions, "function syncPlayerPageDetails() {", "Universal control interactions must not retain Player-only detail synchronization.");
