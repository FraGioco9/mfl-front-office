(() => {
  ...
  const STATIC_RELEASE_VERSION = "1.128.2";
  ...
  function firstPaintClubIdentity(urlLike = window.location.href) {
    ...
    try {
      ...
      return {
        ...
        ownerWalletAddress: String(identity?.ownerWalletAddress || "").trim().toLowerCase(),
      };
    } catch {
  ...
  function primeClubIdentityFirstPaint(urlLike = window.location.href) {
    ...
    const request = canonicalBootstrapRequest(urlLike);
    progression.dataset.clubView = String(request?.options?.view || "attributes");
  ...
  function primeClubProfileLoading(view = "attributes") {
    const progression = document.getElementById("progressionPage");
    const normalizedView = String(view || "attributes");
  ...
  function primeStatsHistogramSkeleton(target) {
    ...
    const previousLabels = Array.from(element.querySelectorAll(":scope > .mflStatsHistogram .mflStatsHistogramLabel"))
      ...
      .filter(Boolean);
    const labels = previousLabels.length > 1
      ? previousLabels
    ...
    const sigma = Math.max(1, labels.length / 5);
    labels.forEach((sample, index) => {
      ...
      const distance = (index - midpoint) / sigma;
      const bellHeight = Math.max(8, Math.round(Math.exp(-0.5 * distance * distance) * 96));
      fill.style.setProperty("--bar-height", `${bellHeight}%`);
      const label = document.createElement("span");
    ...
    playerDetail.innerHTML = `
      <section class="playerHero playerHeroPending" aria-hidden="true">
        <div class="playerHeroMedia">
          ...
          <a class="playerHeroBrandMark" tabindex="-1" aria-hidden="true"><img class="playerHeroBrandLogo" alt="" aria-hidden="true"><svg class="playerHeroDevelopmentCenterIcon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M16.05 10.966a5 2.5 0 0 1-8.1 0"></path><path d="m16.923 14.049 4.48 2.04a1 1 0 0 1 .001 1.831l-8.574 3.9a2 2 0 0 1-1.66 0l-8.574-3.91a1 1 0 0 1 0-1.83l4.484-2.04"></path><path d="M16.949 14.14a5 2.5 0 1 1-9.9 0L10.063 3.5a2 2 0 0 1 3.874 0z"></path><path d="M9.194 6.57a5 2.5 0 0 0 5.61 0"></path></svg></a>
