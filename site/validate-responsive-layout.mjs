...
includes(responsive, ".searchDialog {\n    width: min(680px, calc(100vw - 24px));\n    max-width: calc(100vw - 24px);\n    height: auto;\n    max-height: calc(100dvh - 24px);", "The tablet/mobile search popup must shrink-wrap its exact five-result viewport within the usable screen height.");
...
includes(responsive, ".searchResults {\n    gap: 5px;\n    min-height: 0;\n    height: 290px;\n    max-height: 290px;\n    grid-auto-rows: 54px;", "Search popup results must expose exactly five compact tablet boxes before scrolling.");
includes(responsive, ".searchResult {\n    min-height: 54px;\n    padding: 7px 10px;\n    border-radius: 7px;", "Search result cards must keep the shared desktop-owned text alignment while their tablet dimensions compact.");
...
includes(responsive, ".searchResults {\n    gap: 5px;\n    min-height: 0;\n    height: 290px;\n    max-height: 290px;\n    grid-auto-rows: 54px;", "Tablet Global Search must expose exactly five 54px rows before scrolling.");
includes(responsive, ".searchResult strong {\n    font-size: 13px;\n    line-height: 1.1;", "Tablet Global Search primary text must scale with its result card.");
includes(responsive, ".searchResult span {\n    font-size: 10px;\n    line-height: 1.15;", "Tablet Global Search secondary text must scale with its result card.");
...
includes(
  responsive,
  "#evaluationPage {\n    grid-template-columns: repeat(3, minmax(0, 1fr));\n    align-content: start;",
  "Small-phone Evaluation must start-align implicit grid rows so the shared mobile page floor cannot stretch top controls before Evaluation content is populated.",
);
