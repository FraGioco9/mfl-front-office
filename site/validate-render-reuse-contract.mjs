...
includes(sharedCore, "const evaluationTableRenderReuse = createRenderReuseGuard();", "Evaluation must consume the shared render-reuse guard.");
includes(sharedCore, "function evaluationTableRenderSignature(row) {", "Evaluation must derive a domain-owned render signature.");
for (const input of [
...
const evaluationRenderer = evaluationRendererStart >= 0 && evaluationPageStart > evaluationRendererStart ? sharedCore.slice(evaluationRendererStart, evaluationPageStart) : "";
