...
assert(PROGRESSION_EMAIL_PORTRAIT_HEIGHT_PX === 72, "Progression email portraits must render at exactly 72px high.");
assert(
...
// contain both red and green but no blue. The 100x500 crop scaled to 72px high
// must be 14px wide after rounding, proving width is not forced to 72px.
...
assert(resized?.width === 14 && resized?.height === 72, "The 100x500 crop must become 14x72 while preserving proportions.");
assert(dominantChannel(pixelAt(resized, 7, 12)) === "red", "The upper half must come from the upper part of the top-500px crop.");
assert(dominantChannel(pixelAt(resized, 7, 60)) === "green", "The lower half must come from near row 500 of the source crop.");
assert(dominantChannel(pixelAt(resized, 7, 60)) !== "blue", "Pixels below source row 499 must never enter the email portrait.");
...
assert(silhouetteAligned?.width === 13 && silhouetteAligned?.height === 72, "A 90x500 left-aligned crop, including right transparency, must become 13x72.");
assert(pixelAt(silhouetteAligned, 0, 36)[3] > 0, "The visible silhouette must begin at the output's left edge.");
...
assert(decoded.width === 14 && decoded.height === 72, "Encoded progression email portrait PNG must preserve the cropped aspect ratio at 72px high.");
assert(dominantChannel(pixelAt(decoded, 7, 12)) === "red", "Encoded PNG must preserve the upper crop pixels.");
assert(dominantChannel(pixelAt(decoded, 7, 60)) === "green", "Encoded PNG must preserve pixels near source row 500 after scaling.");

console.log("Progression email portrait validation passed: top-500 crop, silhouette-left alignment, proportional 72px height.");
