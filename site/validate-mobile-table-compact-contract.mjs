...
const mobileTableSource = readFileSync(resolve(root, "modules/app-core-mobile-table.js"), "utf8");
...
assert.match(
  ...
  "Mobile table presentation must retain the shared 900px and compact 520px breakpoints.",
);
...
assert.match(
  mobileTableSource,
  /player_seasons: "SZN"/,
  "Hydrated small-screen Seasons headings must use SZN.",
);
assert.match(
  bootstrapSource,
...
assert.match(
  ...
  "Mobile row text must retain its 12px font contract.",
);
assert.match(
  responsiveSource,
  /@media \(max-width: 520px\)[\s\S]*#progressionPage \.playerTableScroller td \{\n {4}font-size: 11px;\n {2}\}/,
  "Phone row text must retain its 11px font contract.",
);
assert.match(
  responsiveSource,
  /@media \(max-width: 380px\)[\s\S]*#progressionPage \.playerTableScroller td \{\n {4}font-size: 10px;\n {2}\}/,
  "Tiny-screen row text must retain its 10px font contract.",
);
assert.match(
  projectionSource,
  /@media \(max-width: 900px\)[\s\S]*--mfl-table-header-height: 32px; --mfl-table-row-height: 28px; --mfl-table-row-outer-height: 32px;/,
  "First-paint mobile header height must already match the visible row height.",
);
assert.match(
  projectionSource,
  /@media \(max-width: 520px\)[\s\S]*--mfl-table-header-height: 28px; --mfl-table-row-height: 24px; --mfl-table-row-outer-height: 28px;/,
  "First-paint phone header height must already match the visible row height.",
);
assert.match(
  projectionSource,
  /@media \(max-width: 380px\)[\s\S]*--mfl-table-header-height: 26px; --mfl-table-row-height: 22px; --mfl-table-row-outer-height: 26px;/,
  "First-paint tiny-screen header height must already match the visible row height.",
);

assert.match(
  mobileTableSource,
  /selectVisibleInput\.type = "checkbox";[\s\S]*selectVisibleInput\.disabled = true;/,
  "Every rebuilt table header must start with selection disabled until visible data exists.",
);
assert.match(
  tableRuntimeSource,
  /selectVisibleInput\.type = "checkbox";[\s\S]*selectVisibleInput\.disabled = true;/,
  "Generated table runtime must preserve the disabled header-selection first state.",
);
assert.match(
  sharedTableUiSource,
  /#progressionPage #tableHead \.selectionCell input:disabled \{[\s\S]*opacity: 0\.45;/,
  "The disabled hydrated header checkbox must be graphically distinct.",
);
assert.match(
  projectionSource,
  /#tableHead \.selectionCell input:disabled \{ opacity: 0\.45; \}/,
  "The first-paint disabled header checkbox must already use the final disabled appearance.",
);

assert.match(
  mobileTableSource,
  /function compactMobileJoinedAgency\(value\) \{[\s\S]*split\(\/\\\\s\+\/, 1\)\[0\]/,
  "Joined Agency must have a compact date-only formatter.",
);
assert.match(
  mobileTableSource,
  /column === joinedAgencyColumn[\s\S]*window\.matchMedia\("\(max-width: 520px\)"\)\.matches[\s\S]*compactMobileJoinedAgency\(joinedAgencyValue\)/,
  "Joined Agency must switch to date-only values only on small screens.",
);

const phoneStyle = sharedTableUiSource.match(/@media \(max-width: 520px\) \{([\s\S]*?)\n\}\n@media \(max-width: 380px\)/)?.[1] || "";
assert.match(
  phoneStyle,
  /#progressionPage #tableBody \.tableOverallRarityCircle \{[\s\S]*flex-basis: 5px;[\s\S]*width: 5px;[\s\S]*height: 5px;[\s\S]*margin-right: 3px;/,
  "The Overall rarity circle must use the refined 5px size and 3px number gap on phone screens.",
);
const tinyStyle = sharedTableUiSource.match(/@media \(max-width: 380px\) \{([\s\S]*?)\n\}`;/)?.[1] || "";
assert.match(
  tinyStyle,
  /#progressionPage #tableBody \.tableOverallRarityCircle \{[\s\S]*flex-basis: 5px;[\s\S]*width: 5px;[\s\S]*height: 5px;[\s\S]*margin-right: 3px;/,
  "The Overall rarity circle must keep the refined 5px size and 3px number gap on tiny screens.",
);

assert.doesNotMatch(mobileTableSource, /!important/, "Mobile compact renderer presentation must not add !important overrides.");
assert.doesNotMatch(sharedTableUiSource, /!important/, "Shared mobile table presentation must not add !important overrides.");

console.log("Mobile compact table contract validation passed.");
