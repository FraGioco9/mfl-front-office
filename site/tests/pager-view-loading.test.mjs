...
test("table loading runtime owns pager visibility for view changes", async () => {
  ...
  assert.match(source, /function dataLoadingActive\(\)[\s\S]*mflDataLoading/);
  ...
  assert.match(source, /function releasePagerWhenReady\(\)[\s\S]*pagerObservedDataLoading/);
  ...
  assert.match(source, /attributeFilter: \["hidden", "data-page", "class"\]/);
