...
test("static app bridge owns the current footer before asynchronous runtime", async () => {
  ...
  assert.equal(release.version, "1.123.34");
  assert.match(index, /<body data-page="home" class="pinnedSidebarVisible">/);
  assert.match(index, />MFL Front Office v1\.123\.34<\/a>/);
  assert.match(index, /href="\/styles\.css\?v=1\.123\.34"/);
  assert.match(index, /src="\/app\.js\?v=1\.123\.34"/);
