...
from scripts.email import send_progression_emails as emails
from scripts.email.player_portraits import canonical_player_portrait_url
...
class ProgressionEmailPortraitTests(unittest.TestCase):
    ...
    def test_html_uses_responsive_portrait_with_intrinsic_ratio(self) -> None:
        ...
        self.assertEqual(emails.EMAIL_PORTRAIT_DISPLAY_HEIGHT_PX, 88)
        self.assertEqual(emails.EMAIL_PORTRAIT_MOBILE_DISPLAY_HEIGHT_PX, 32)
        self.assertEqual(emails.EMAIL_PORTRAIT_COMPACT_DISPLAY_HEIGHT_PX, 24)
        self.assertIn('class="email-player-portrait"', identity)
        self.assertIn('width:auto;max-width:none', identity)
        self.assertNotIn('height:88px', identity)
        self.assertIn('height="88"', identity)
        self.assertIn('margin:0 auto', identity)

        rendered = emails.build_html("Test Email", [self.player(portrait_url=EMAIL_PORTRAIT_123)])
        self.assertIn('.email-player-portrait { height:88px; width:auto; max-width:none; }', rendered)
        self.assertIn('.email-player-portrait { height:32px; }', rendered)
        self.assertIn('.email-player-portrait { height:24px; }', rendered)
        self.assertEqual(rendered.count('.email-player-portrait { height:'), 3)
    ...
    def test_player_column_uses_percentage_portrait_slot(self) -> None:
        ...
        self.assertIn('width:38%;padding:0 4% 0 0;overflow:hidden;text-align:center;', identity)
        self.assertIn('align="center"', identity)
