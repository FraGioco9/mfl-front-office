...
if [ ! -f production-site/api/_database.js ]; then
...
rm -rf production-site/api/data-files
mkdir -p production-site/api/data-files
cp "$DATABASE_SOURCE_PATH" production-site/api/data-files/mfl_database.db
test -s production-site/api/data-files/mfl_database.db
...
if find production-site/api/data-files -maxdepth 1 -type f -name '*.json' | grep -q .; then
