...
const headerRule = (headers, source, predicate = () => true) =>
  headers.find((rule) => rule?.source === source && predicate(rule));
...
for (const [name, headers] of [
  ...
  const cssPolicy = cacheControl(headerRule(headers, "/:path*.css"));
  invariant(
...
const developmentJsPolicy = cacheControl(headerRule(developmentHeaders, "/:path*.js"));
invariant(
