...
assert.ok(source.includes("const y = 76 - lineIndex * (66 / (lines.length - 1));") && generated.includes("const y = 76 - lineIndex * (66 / (lines.length - 1));") && source.includes('goalkeeper.style.top = "94%";'), "All formations must use the upper and lower pitch evenly in canonical and generated shells.");
assert.ok(css.includes("width:clamp(48px,14%,62px)") && styles.includes("width:clamp(48px,14%,62px)") && css.includes("width:clamp(44px,14%,56px)") && styles.includes("width:clamp(44px,14%,56px)"), "Smaller circles must match across desktop/mobile and canonical/generated styles.");
assert.ok(source.includes("count >= 5 ? 82 : count === 4 ? 80 : count === 3 ? 68 : 48") && generated.includes("count >= 5 ? 82 : count === 4 ? 80 : count === 3 ? 68 : 48"), "Use the full available pitch width for each formation line.");
