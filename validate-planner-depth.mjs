...
assert.ok([css, styles].every(sheet => sheet.includes(".plannerFormationPositionLabel{position:absolute;top:calc(100% + 8px);left:50%;display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box;max-width:70px;min-height:16px;padding:0 5px;overflow:hidden;border-radius:5px;background:rgb(162,162,162);color:var(--text);font-size:12px;font-weight:700;letter-spacing:.4px;line-height:16px;text-align:center;text-overflow:ellipsis;text-shadow:none;white-space:nowrap;transform:translateX(-50%);pointer-events:none}") && !sheet.includes(".plannerFormationSpot .plannerFormationPositionLabel{top:calc(86% + 1px)}")), "All empty slots must use compact grey position badges at the occupied surname distance.");
