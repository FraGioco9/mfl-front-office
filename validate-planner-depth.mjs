...
assert.ok([source, generated].every(shell => shell.includes("depthPickerCandidates(depthRoster, position, new Set(assignmentByPlayer.keys()), currentId);")
  ...
  && shell.includes('removeIconFrame.className = "plannerDepthPickerRemoveFrame";')
  && shell.includes('removeIconFrame.appendChild(removeIcon);')
  && shell.includes('clear.append(removeIconFrame, removeText);')
...
assert.ok([css, styles].every(sheet => sheet.includes('.plannerDepthActions{display:flex;align-items:center;gap:6px;margin-left:auto}')
  ...
  && sheet.includes('.plannerDepthPickerPointer{position:absolute;z-index:1;top:-9px;width:16px;height:10px;pointer-events:none}')
  && sheet.includes('.plannerDepthPickerPointer::after{content:"";position:absolute;top:2px;left:2px;box-sizing:border-box;width:12px;height:12px;border:1px solid var(--border-strong);border-right:0;border-bottom:0;background:var(--surface);transform:rotate(45deg)}')
...
assert.ok([source, generated].every(shell => shell.includes('const standardMidfieldLine = flatFourMidfield || (midfield && count === 3')
  && shell.includes('((selected === "4312" || selected === "4321") && linePositions.every(position => position === "CM"))')
  && shell.includes('(selected === "4132" && linePositions.join(",") === "LM,CM,RM")')), "4-3-1-2, 4-3-2-1 and 4-1-3-2 must use the 4-4-2 midfield baseline without changing other formations.");
