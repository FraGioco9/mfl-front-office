    ...
    if (isNewIdentifierSyntax(cadence)) {
      for (const [fullMatch, contractName] of getContractIdentifierSyntaxMatches(cadence)) {
        ...
        if (address) {
          ...
          const canonical = context.contracts[contractName + ".canonical"] || null;
          ...
          const importStatement = canonical ? `import ${canonical} as ${contractName} from ${utilAddress.withPrefix(address)}` : `import ${contractName} from ${utilAddress.withPrefix(address)}`;
...
// Use "1.13.7" if available (from build), otherwise use a default for development
