import { spawn, UNSUBSCRIBE, SUBSCRIBE, send, subscriber } from '@onflow/util-actor';
 ...
 */
const mapContractAliasesToNetworkAddress = (network, fallbackNetwork) => contracts => {
  return Object.entries(contracts).reduce((c, _ref) => {
    let [key, value] = _ref;
    const alias = value?.aliases?.[network] || fallbackNetwork && value?.aliases?.[fallbackNetwork];
    if (alias) {
      c[key] = alias;
    }
    return c;
 ...
 */
const mapDependencyAliasesToNetworkAddress = (network, fallbackNetwork) => dependencies => {
 ...
 */
const mapContractCanonicals = contracts => {
  return Object.entries(contracts).reduce((c, _ref3) => {
    let [key, value] = _ref3;
    if (value?.canonical) {
 ...
 */
const mapDependencyCanonicals = dependencies => {
  return Object.entries(dependencies).reduce((c, _ref4) => {
    let [key, value] = _ref4;
    if (value?.canonical) {
      c[`${key}.canonical`] = value.canonical;
    }
...
const mapDeploymentsToNetworkAddress = network => _ref5 => {
  let {
    deployments = {},
    accounts = {}
  } = _ref5;
  ...
  return Object.entries(networkDeployment).reduce((c, _ref6) => {
    let [key, value] = _ref6;
...
const getContracts = (jsons, network) => {
  ...
  const fallbackNetwork = networkConfig && typeof networkConfig === "object" && "fork" in networkConfig ? networkConfig.fork : undefined;
  return mergePipe(mapDeploymentsToNetworkAddress(network), pipe(filterContracts, mapContractAliasesToNetworkAddress(network, fallbackNetwork)), pipe(filterDependencies, mapDependencyAliasesToNetworkAddress(network, fallbackNetwork)), pipe(filterContracts, mapContractCanonicals), pipe(filterDependencies, mapDependencyCanonicals))(mergedJson);
};
...
const hasPrivateKeys = flowJSON => {
  return Object.entries(flowJSON?.accounts ?? []).reduce((hasPrivateKey, _ref7) => {
    let [, value] = _ref7;
...
const cleanNetwork = network => {
  return network?.toLowerCase() === "local" ? "emulator" : network?.toLowerCase();
};
...
export { clearConfig, config, getContracts };
