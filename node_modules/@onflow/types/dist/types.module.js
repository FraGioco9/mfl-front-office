...
const UFix128 = typedef("UFix128", v => {
  if (isString(v)) {
    const vParts = v.split(".");
    if (vParts.length !== 2) {
      return throwTypeError(`Expected one decimal but found ${vParts.length} in the [U]Fix128 value. Find out more about [U]Fix128 types here: https://cadencelanguage.com/docs/language/values-and-types/fixed-point-nums-ints`);
    }
    if (vParts[1].length == 0 || vParts[1].length > 24) {
      return throwTypeError(`Expected at least one digit, and at most 24 digits following the decimal of the [U]Fix128 value but found ${vParts[1].length} digits. Find out more about [U]Fix128 types here: https://cadencelanguage.com/docs/language/values-and-types/fixed-point-nums-ints`);
    }
...
export { Address, _Array as Array, Bool, Character, Dictionary, Enum, Event, Fix128, Fix64, Identity, InclusiveRange, Int, Int128, Int16, Int256, Int32, Int64, Int8, Optional, Path, Reference, Resource, String, Struct, UFix128, UFix64, UInt, UInt128, UInt16, UInt256, UInt32, UInt64, UInt8, Void, Word128, Word16, Word256, Word32, Word64, Word8, _Array };
