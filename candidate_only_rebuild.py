...
def install_candidate_only_rebuild(rebuild_module: ModuleType) -> None:
    ...
    def candidate_print(*args: Any, **kwargs: Any) -> None:
        if args and isinstance(args[0], str) and args[0].startswith("Flow database rebuild complete:"):
            ...
            previous_print(f"Current database unchanged: {database}", *args[1:], **kwargs)
            return
